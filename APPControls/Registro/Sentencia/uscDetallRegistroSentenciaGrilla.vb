Imports Bussines.Mantenimiento
Imports Type.Enumeracion.TipoDocJudicial
Namespace Registro.Sentencias
    Public Class uscDetallRegistroSentenciaGrilla
        '/*bussiness*/
...
#Region "Propiedades_Parametricas"
        ...
        Private Property DocumentoFechaRecepcion() As Long
            ...
            End Set
        End Property
        Private Property DocumentoFechaSentencia() As Long
            Get
                Return dtpFechSenDoc.ValueLong
            End Get
            Set(value As Long)
                dtpFechSenDoc.ValueLong = value
            End Set
        End Property

        Public Property DistritoJudicial() As String
            Get
                Return txtDistritoJudicial.Text
            End Get
            Set(value As String)
                txtDistritoJudicial.Text = value
            End Set
        End Property
        Public Property AutoridadJudicial() As String
            Get
                Return txtAutoridadJudicial.Text
            End Get
            Set(value As String)
        ...
        Public Property _CadenaPerpetua() As Boolean
            ...
            Set(ByVal value As Boolean)
                rbtCadenaperpetua.Checked = value
            End Set
        End Property
        ...
        Private Sub Temporal_Grabar_Expediente_Delito_Agraviado(objEntExpedienteCol As Entity.Registro.ExpedienteCol, objEntDelitoCol As Entity.Registro.DelitoCol,
            ...
            For Each objEntExpediente As Entity.Registro.Expediente In objEntExpedienteCol
                ...
                With objEntExpediente
                    Me.DocumentoNumero = .DocumentoJudicialNum
                    Me.DocumentoFechaRecepcion = .DocFechaDocRecepcion
                    Me.DocumentoFechaSentencia = .FechaSentencia
                    Me.DistritoJudicial = .DocumentoDistritoJudicial
                    Me.AutoridadJudicial = .DocumentoAutoridadJudicial
                    '-----------Detalle-----------
                    Select Case .MotivoIngresoID
                        ...
                        Case Else
                            rdbNo.Checked = True
                    End Select
                    Me._CadenaPerpetua = .SentenciaCadenaPerpetua
                    If Me._CadenaPerpetua = False Then Me.rbtTemporal.Checked = True
                    Me._TotalAnhos = .SentenciaAnio
                    Me._TotalMes = .SentenciaMes
                    Me._TotalDias = .SentenciaDia
                    Me.FechaInicio = .SentenciaFechaInicio
                    Me.FechaFinal = .SentenciaFechaFinal
                End With
                '--------------------
            ...
            Else
                ...
                For Each objDel As Entity.Registro.Delito In objEntDelitoTempoColGrilla
                    ...
                    For Each objAgr As Entity.Registro.Agraviado In objEntAgraviadoTempoColGrilla
                        ...
                        f.Cells("col_del_art").Value = objDel.DelitoArticulo
...
#Region "Eliminar"
        Private Sub Temporal_EliminarExpediente_Delito_Agraviado(SelectExpediente As Boolean, SelectDelito As Boolean, SelectAgraviado As Boolean)
            ...
            If SelectExpediente = True Then
                ...
                If Legolas.Configuration.Aplication.MessageBox.QuestionEliminar(strMensaje) = DialogResult.Yes Then
                    ...
                    '--Limpiar Cabecera-------
                    DocumentoNumero = ""
                    DocumentoFechaRecepcion = Nothing
                    DocumentoFechaSentencia = Nothing
                    DistritoJudicial = ""
                    AutoridadJudicial = ""
                    rdbNo.Checked = True
        ...
        Public Function Validar() As Boolean
            ...
            If rbtTemporal.Checked = True And (Me._TotalAnhos.Length = 0 And Me._TotalMes.Length = 0 And Me._TotalDias.Length = 0) And (FechaInicio = 0 And FechaFinal = 0) Then
                Legolas.Configuration.Aplication.MessageBox.Exclamation("Ingrese fecha documento de Recepción del documento")
                Me.txtAnios.Focus()
                Return blnValue
            End If

            If Me.FechaInicio > 0 And FechaInicio > FechaFinal And FechaFinal > 0 Then
...
#Region "Delito"
        ...
        Private Sub Temporal_DelitoCopiarPropiedades(objEntOrigen As Entity.Registro.Delito, ObjEntDestino As Entity.Registro.Delito)
            With objEntOrigen
                ...
                .DelitoEspecificoNombre = ObjEntDestino.DelitoEspecificoNombre & " / " & .DelitoGenericoNombre
                .DelitoArticulo = ObjEntDestino.DelitoArticulo
...
#Region "Listar"
        ...
        Public Sub _ListarExpediente(ObjDocJudicial As Entity.Registro.DocumentoJudicial, intIDDocumentoJudTipo As Short, Optional ByVal CargarMandDetencionHistorico As Boolean = True,
            ...
            If Me._DocumentoJudicialID < 1 Then 'si es nuevo y se muestra los checkeds
                ...
                'nada
            Else
                If Me._TipoResolucion = 72 Then
                    With ObjDocJudicial
                        If ObjDocJudicial.SentenciaCadenaPerpetua = True Then
                            rbtCadenaperpetua.Checked = True
                        Else
                            rbtTemporal.Checked = True
                        End If
                        txtAnios.Text = .SentenciaTotalAnios
                        txtMeses.Text = .SentenciaTotalMeses
                        txtDias.Text = .SentenciaTotalDias
                    End With
                End If
                ...
                For Each objEntExpediente As Entity.Registro.Expediente In objEntExpedienteColTmp
                    ...
                    Select Case objEntExpediente.DocumentoTipoID
                        ...
                        Case 72
                            If objEntExpediente.ExpedientePadreReferidoID > 0 Then
                                Dim objExpAux As New Entity.Registro.Expediente
                                Dim objExpAuxCol As New Entity.Registro.ExpedienteCol
                                objExpAux.Codigo = objEntExpediente.ExpedientePadreReferidoID
                                objExpAux.InternoID = objEntExpediente.InternoID
                                objBssExp = New Bussines.Registro.Expediente
                                objExpAuxCol = objBssExp.ListarGrillaSentenciasNoConsentidas(objExpAux)
                                Me.DocumentoNumero = objExpAuxCol.Expediente(0).DocumentoJudicialNum
                                Me.txtDistritoJudicial.Text = objExpAuxCol.Expediente(0).DocumentoDistritoJudicial
                                Me.txtAutoridadJudicial.Text = objExpAuxCol.Expediente(0).SalaJudicialNombre
                                Me.dtpFechRecepDoc.ValueLong = objExpAuxCol.Expediente(0).DocFechaDocRecepcion
                                Me.dtpFechSenDoc.ValueLong = objExpAuxCol.Expediente(0).FechaSentencia
                            End If

                            If objEntExpediente.MotivoIngresoID = 178 Then
                                rdbSi.Checked = True
                                txtEstablecimientomental.Text = objEntExpediente.Establecimientomental
                            End If
                            Me.dtpFechaInicio.ValueLong = objEntExpediente.FechaInicio
                            Me.dtpFechaFinal.ValueLong = objEntExpediente.FechaFinal
                    End Select
                    Temporal_GrabarExpediente(objEntExpediente)
...
#Region "Accion"
        ...
        Public Sub _Grabar(Nuevo As Boolean, EntDoc As Entity.Registro.DocumentoJudicial)
            EliminarExpedienteBD()
            AGrabar(Nuevo, EntDoc)
        End Sub
        Private Sub AGrabar(Nuevo As Boolean, EntDoc As Entity.Registro.DocumentoJudicial)
            Dim DocumentoJudicialID As Integer = EntDoc.Codigo
            ...
            With entPerCond
                ...
                .FechaFin = Me.FechaFinal
                .PenalId = Me._PenalID
                .RegionId = Me._RegionID
            End With
            If Nuevo = True Then
                If Me.FechaInicio > 0 Or Me.FechaFinal > 0 Or Me._TotalAnhos <> "0" Or Me._TotalMes <> "0" Or Me._TotalDias <> "0" Then
            ...
            Else
                If objEntPeriodoCondenaTempoCol.Count > 0 Then
                    entPerCond.Codigo = objEntPeriodoCondenaTempoCol.PeriodoCondenaSentencia(0).Codigo
                End If
                If Me.FechaInicio > 0 Or Me.FechaFinal > 0 Or Me._TotalAnhos <> "0" Or Me._TotalMes <> "0" Or Me._TotalDias <> "0" Then
                    If Me._TipoLicencia = Type.Enumeracion.Licencia.enmTipoLicencia.PenalLimaMetropolitana Or Me._TipoLicencia = Type.Enumeracion.Licencia.enmTipoLicencia.RegionLimaMetropolitana Or Me._TipoLicencia = Type.Enumeracion.Licencia.enmTipoLicencia.Carceleta Then
                        objBssPerCon.Grabar_LM(entPerCond)
            ...
            For Each objExpediente As Entity.Registro.Expediente In Me.objEntExpedienteTempoCol
                ...
                If Me._TipoResolucion = Type.Enumeracion.TipoDocJudicial.enumTipoDocumentoJudicial.Sentencia Or
                   Me._TipoResolucion = enumTipoDocumentoJudicial.ResolucionDeclaraConsentida Then
                    objExpediente.SituacionJuridicaID = Type.Enumeracion.SituacionJuridica.EnumSituacionJuridica.Sentenciado
                End If
...
#End Region
        ...
        Private Sub dgvPeriodoCondena_CellDoubleClick(sender As Object, e As System.Windows.Forms.DataGridViewCellEventArgs)
            If dgvPeriodoCondena.Rows.Count < 1 Then Exit Sub
            btnEditPerCond_Click(Nothing, Nothing)
        End Sub

        Private Sub btnEditPerCond_Click(sender As System.Object, e As System.EventArgs)
        ...
        Private Sub btnDelPerCond_Click(sender As System.Object, e As System.EventArgs)
            If dgvPeriodoCondena.Rows.Count < 1 Then
                Legolas.Configuration.Aplication.MessageBox.Exclamation("Seleccionar un periodo de condena para eliminar")
                Exit Sub
            End If
        ...
        Private Sub btnDelDelito_Click(sender As System.Object, e As System.EventArgs)
            If dgwDelito.Rows.Count = 0 Then
        ...
        Private Sub btnEditDelito_Click(sender As System.Object, e As System.EventArgs)
            bolEdicionActivo = False
            Temporal_Delito_Modificar()
            bolEdicionActivo = True
        End Sub

        Private Sub dgwDelito_CellDoubleClick(sender As Object, e As System.Windows.Forms.DataGridViewCellEventArgs)
        ...
        Private Sub uscDetallRegistroSentencia_Load(sender As Object, e As System.EventArgs) Handles Me.Load
            ...
            Me.txtDistritoJudicial.Text = ""
            Me.txtAutoridadJudicial.Text = ""
        End Sub
        ...
        Private Sub dgvPeriodoCondena_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs)
            If e.KeyCode = Keys.Delete Then
        ...
        Private Sub txtAnios_Click(sender As Object, e As System.EventArgs)
            txtAnios.Focus()
        End Sub

        Private Sub txtAnios_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs)
        ...
        Private Sub dgwDelito_CellContentClick(sender As Object, e As DataGridViewCellEventArgs)
        ...
        Private Sub rbtTemporal_CheckedChanged(sender As Object, e As EventArgs) Handles rbtTemporal.CheckedChanged
        ...
        Private Sub btnInimputable_Click(sender As Object, e As EventArgs)
        ...
        Private Sub rdbSi_CheckedChanged(sender As Object, e As EventArgs) Handles rdbSi.CheckedChanged
            If rdbSi.Checked = True Then
                pnlObsInimputable.Visible = True
            Else
                pnlObsInimputable.Visible = False
            End If
        End Sub
